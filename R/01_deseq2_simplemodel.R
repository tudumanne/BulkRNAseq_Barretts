#This script contains the R code used to run differential gene expression analysis using DESeq2 (simple model)
#Input files: gene-level read count data from STAR, sample metadata, and gene annotations
...
#design is the variable of interest, i.e. Classification/disease stage
...
write.csv(res, "raw_counts.csv", sep=",")
...
#run DESeq
design(dds) <- ~ 1 + Classification
dds <- DESeq(dds)

#check the coefficients estimated by DEseq
resultsNames(dds)

#edited gene symbol list
#tx2gene <- read.csv("tx2gene.gencode.v36.pa_edited.csv", header=T)
...
norm_counts_untransformed_symbols <- merge(unique(gene.annotation[,1:2]), data.frame(ID=rownames(norm_counts_untransformed), norm_counts_untransformed), by=1, all=F)
norm_counts_log_symbols <- merge(unique(gene.annotation[,1:2]), data.frame(ID=rownames(norm_counts_log), norm_counts_log), by=1, all=F)
norm_counts_untransformed_symbols$gene.id <- sub('\\.[0-9]*$', '', norm_counts_untransformed_symbols$gene.id)
norm_counts_log_symbols$gene.id <- sub('\\.[0-9]*$', '', norm_counts_log_symbols$gene.id)

#write normalized counts to text files
write.table(norm_counts_untransformed_symbols, "normalized_counts.txt", quote=F, col.names=T, row.names=F, sep="\t")
write.table(norm_counts_log_symbols, "normalized_counts_log.txt", quote=F, col.names=T, row.names=F, sep="\t")
...
for(name in names(deg_lists)){
  write.csv(
    ...
    file = file.path("DEG_lists/simple_model/all_genes",
                     paste0(name, "_allgenes.csv"))
  )
}
...
for(name in names(deg_lists)){
  ...
  write.csv(
    as.data.frame(sig),
    file = file.path("DEG_lists/simple_model/sig_FC1",
                     paste0(name, "_sigFC1.csv"))
  )
}

#save DESeq2 results object
saveRDS(dds, "deseq2_simplemodel_dds.rds")
